...
let endPoint="http://localhost:1200/users"
...
export default function Property() {
  ...
  useEffect(() => {
    ...
    // })
    const deletedata = (id)=> {
      axios.delete(`http://localhost:1200/users/${id}`).then((response) => {
        getalldata()
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (isEdit) {
          let mainobj={
            username:RenterObj.username,
            password:RenterObj.password
          }
